...
// node-worker/src/lib/peer.ts). puter.surf is the browser UI that knows how to reach
// those — give it a localhost URL and it resolves the port against the peer servers
...
//
//   - signed in: the credential is the user's auth token, which puter.surf already has
//     from being signed in as the same user. Nothing to pass.
//   - anonymous: the credential is this page's peer token, which puter.surf cannot
//     know, so it travels in the link as `peerToken` and puter.surf presents it to the
//     signaller as its own `anonToken` — the signaller then matches the two by
//     `(token, port)`.

const PUTER_SURF = "https://puter.surf/";
...
/**
 * The puter.surf link for a port, `peerToken` set when the workspace is anonymous.
 *
 * `openUrl` is the parameter puter.surf reads; `peerToken` is the only difference between
 * the two cases, and it is what lets it find a peer server that no user owns.
 */
export function previewUrlFor(port: number, path = "/", peerToken?: string): string {
	...
	return `${PUTER_SURF}?openUrl=${target}${anon}`;
}

/** Rewrite a localhost URL for puter.surf, or return undefined if it isn't one. */
